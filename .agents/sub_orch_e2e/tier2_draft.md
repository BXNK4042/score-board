# Tier 2 - Boundary & Corner Cases Draft Spec
...
test.describe('Tier 2 - Boundary & Corner Cases', () => {
  ...
  test('7. Setup - Unique colors enforced: 8-color palette available', async ({ page }) => {
    await page.getByRole('button', { name: 'Create New Game' }).click();
    await page.getByRole('button', { name: 'ADD PLAYER' }).click();
    // Verify there are at least 8 buttons with color roles in the color picker grid
    const colorButtons = page.locator('form button[type="button"]').filter({ hasText: '' });
  ...
  test('14. In-Game - Add Player mid-game cannot exceed 10 players total', async ({ page }) => {
    ...
    await expect(page.getByRole('button', { name: 'Add player mid-game' }).or(page.getByLabel('Add Player'))).toBeDisabled().or(
      expect(page.getByRole('button', { name: 'Add player mid-game' }).or(page.getByLabel('Add Player'))).not.toBeVisible()
    );
  });
