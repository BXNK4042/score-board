...
28. **Preventing Element Overlap and Layout Height Bugs on iOS**:
    - **Avoid layout-independent status badges**: Do not render status or helper text elements (e.g., `Last Score Updated`) with absolute/negative top margins or large bottom margins directly under a `justify-between` page wrapper. On short lists or short viewports, the distributed space can force these elements to float upwards and overlap other interactive cards.
    - **Position helper status inside the scrolling context**: Place helper status badges inline as the last element of the primary scrollable list container. This preserves natural document block flow, ensures the badge scrolls out of the way when the list is long, and leverages the container's smooth bottom paddings (e.g., `pb-24` for bulk bar spacing) without custom margin hacks.
    - **Avoid restricting root viewport height**: Do not force strict height locking (such as `h-screen` or `overflow-hidden` on the root container) unless a viewport-locked layout is explicitly required. Restricting root heights makes the viewport feel too short or cramped on smaller device screens. Prefer letting components define their own natural scrolling boundaries.